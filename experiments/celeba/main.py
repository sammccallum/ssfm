...
def _process_image_bytes(args):
    raw_bytes, resolution = args
    try:
        img = Image.open(io.BytesIO(raw_bytes)).convert("RGB")
        img = img.resize((resolution, resolution), Image.BICUBIC)  # pyright: ignore
...
def celeba(path="experiments/celeba/data", resolution=64):
    file_id = "0B7EVK8r0v71pZjFTYXZWM3FlRnM"
    zip_path = os.path.join(path, "img_align_celeba.zip")
    cache_path = os.path.join(path, f"celeba_{resolution}x{resolution}.npz")

    os.makedirs(path, exist_ok=True)

    if os.path.exists(cache_path):
        print(f"Loading cached CelebA {resolution}x{resolution} from {cache_path}")
        data = np.load(cache_path)
        return data["images"], data["data_mean"], data["data_std"]

    if not os.path.exists(zip_path):
        print("Downloading CelebA...")
        gdown.download(id=file_id, output=zip_path)  # pyright: ignore

    print("Loading images from zip archive...")
    with zipfile.ZipFile(zip_path, "r") as zf:
        members = sorted(
            n for n in zf.namelist() if n.lower().endswith((".jpg", ".jpeg", ".png"))
        )
        image_bytes = [zf.read(name) for name in members]
    print(f"Decoding and resizing {len(image_bytes)} images...")
    args = [(b, resolution) for b in image_bytes]
    with ThreadPoolExecutor(max_workers=8) as pool:
        results = list(pool.map(_process_image_bytes, args))
    del image_bytes

    all_images = [r for r in results if r is not None]
    del results
    images = np.stack(all_images)
    del all_images

    data_mean = np.mean(images, axis=0)
    data_std = np.clip(np.std(images, axis=0), a_min=1e-6, a_max=None)
    images = ((images - data_mean) / data_std).astype(np.float32)

    np.savez(cache_path, images=images, data_mean=data_mean, data_std=data_std)
...
def train_step(
    ...
    y0_batch: Float[Array, "batch 3 64 64"],
...
def main():
    ...
    checkpoint_dir = "experiments/celeba/checkpoints"
    warmup_steps = 10_000
    sample_step_sizes = [1 / 16, 1 / 8, 1 / 4, 1 / 2, 1]
    ...
    data_mean = jnp.array(data_mean.reshape(data_shape))
    data_std = jnp.array(data_std.reshape(data_shape))
    print(f"Dataset: {dataset.shape} (numpy on CPU)")
    ...
    for step in range(start_step, n_train_steps):
        ...
        idx = rng_data.integers(0, dataset.shape[0], size=(batch_size,))
        y0_batch = jax.device_put(dataset[idx], data_sharding)
    ...
    fig.savefig("experiments/celeba/celeba_edm2_em_samples.png", dpi=150)
    print("Saved experiments/celeba/celeba_edm2_em_samples.png")
